...
def update_bite_count(user_id):
    rows = []
    with open('bite.csv', 'r', newline='') as file:
        reader = csv.DictReader(file)
        rows = list(reader)
        fieldnames = reader.fieldnames
    updated = False
    for row in rows:
        if row['id'] == user_id:
            if 'bite' not in fieldnames or not row['bite'].isdigit():
                row['bite'] = '1'
            else:
                bite_count = int(row['bite'])
                bite_count += 1
                row['bite'] = str(bite_count)
            updated = True
            break
    if not updated:
        new_row = {'id': user_id, 'bite': '1'}
        rows.append(new_row)
    with open('bite.csv', 'w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)
...
def reset_temp():
    ...
    with open("bite_temp.csv", "w", newline="") as temp_file:
        writer = csv.writer(temp_file)
        writer.writerow(["id", "bite"])
    os.remove("bite.csv")
    os.replace("bite_temp.csv", "bite.csv")
...
def check_bite_count(id_number):
    wolf_num = check_werewolf_num()
    with open('bite.csv', 'r', newline='') as file:
        reader = csv.DictReader(file)
        for row in reader:
            if row['id'] == id_number:
                bite_count = int(row['bite'])
                if bite_count == wolf_num:
                    return True
    return False
